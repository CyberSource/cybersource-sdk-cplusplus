...
ics_msg *convertSimpleOrderResponseToICSResponse(std::map <std::wstring, std::wstring> soResponse){
   ...
   //TODO: Use a lookup table for the mapping
    ...
    ics_fadd(icsResponse, "currency", soResponse[L"purchaseTotals_currency"]);
    ics_fadd(icsResponse, "auth_card_pinless_debit", soResponse[L"ccAuthReply_cardPINlessDebit"]);
    ics_fadd(icsResponse, "auth_auth_code", soResponse[L"ccAuthReply_authorizationCode"]);
    ics_fadd(icsResponse, "auth_card_regulated", soResponse[L"ccAuthReply_cardRegulated"]);
    ics_fadd(icsResponse, "auth_card_payroll", soResponse[L"ccAuthReply_cardPayroll"]);
    ics_fadd(icsResponse, "auth_card_commercial", soResponse[L"ccAuthReply_cardCommercial"]);
    ics_fadd(icsResponse, "card_type", soResponse[L"card_cardType"]);
    ics_fadd(icsResponse, "auth_card_issuer_country", soResponse[L"ccAuthReply_cardIssuerCountry"]);
    ics_fadd(icsResponse, "auth_card_healthcare", soResponse[L"ccAuthReply_cardHealthcare"]);
    ics_fadd(icsResponse, "auth_card_level_3_eligible", soResponse[L"ccAuthReply_cardLevel3Eligible"]);
    ics_fadd(icsResponse, "ics_rcode", soResponse[L"decisionEarlyReply_rcode"]);
    ics_fadd(icsResponse, "auth_card_signature_debit", soResponse[L"ccAuthReply_cardSignatureDebit"]);
    ics_fadd(icsResponse, "auth_card_prepaid", soResponse[L"ccAuthReply_cardPrepaid"]);
    ics_fadd(icsResponse, "auth_auth_response", soResponse[L"ccAuthReply_processorResponse"]);
    ics_fadd(icsResponse, "auth_payment_network_transaction_id", soResponse[L"ccAuthReply_paymentNetworkTransactionID"]);
    ics_fadd(icsResponse, "terminal_id", soResponse[L"pos_terminalID"]);
    ics_fadd(icsResponse, "auth_auth_avs", soResponse[L"ccAuthReply_avsCode"]);
    ics_fadd(icsResponse, "auth.reason_code", soResponse[L"ccAuthReply_reasonCode"]);
    ics_fadd(icsResponse, "merchant_ref_number", soResponse[L"merchantReferenceCode"]);
    ics_fadd(icsResponse, "auth_trans_ref_no", soResponse[L"ccAuthReply_reconciliationID"]);
    ics_fadd(icsResponse, "auth_auth_amount", soResponse[L"ccAuthReply_amount"]);
    ics_fadd(icsResponse, "request_id", soResponse[L"requestID"]);
    ics_fadd(icsResponse, "auth_auth_time", soResponse[L"ccAuthReply_authorizedDateTime"]);
    ics_fadd(icsResponse, "request_token", soResponse[L"requestToken"]);
    ics_fadd(icsResponse, "auth_affluence_indicator", soResponse[L"ccAuthReply_affluenceIndicator"]);
    ics_fadd(icsResponse, "auth_avs_raw", soResponse[L"ccAuthReply_avsCodeRaw"]);
    ics_fadd(icsResponse, "ics_decision_reason_code", soResponse[L"decisionEarlyReply_reasonCode"]);
